...
if hasattr(st, 'secrets') and len(st.secrets) > 0 and "OPENAI_API_KEY" in st.secrets:
    ...
    config_content = {
        ...
        "search": {
            "api_type": st.secrets.get("SEARCH_ENGINE", "ddg"),  # 默认使用免费的 DuckDuckGo
    ...
    # 设置 MetaGPT 配置路径
    os.environ["METAGPT_CONFIG_PATH"] = temp_config.name

    st.sidebar.success("✅ 使用 Streamlit Cloud Secrets 配置")
elif os.path.exists("config.yaml"):
    # 本地开发环境，使用 config.yaml
    os.environ["METAGPT_CONFIG_PATH"] = "config.yaml"
    st.sidebar.info("🔧 使用本地 config.yaml 配置")
else:
    st.error("⚠️ **配置缺失**")
    st.markdown("""
    请选择以下方式之一进行配置:

    **方式 1: Streamlit Cloud 部署**
